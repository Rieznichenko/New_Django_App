...
def thread_store_get(channel_id):
    with open("thred_store_file.json", 'r') as file:
        data = json.load(file)

    print("Jso data", data)
    get_thread = data.get(str("2"), None)
    print("Thread store get", get_thread)
    return get_thread

def thread_store_put(channel_id, thread_id):
    with open("thred_store_file.json", 'r') as file:
        data = json.load(file)

    data[channel_id] = thread_id
    with open("thred_store_file.json", 'w') as file:
